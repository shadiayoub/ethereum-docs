...
### Choosing an approach <a href="#choosing-approach" id="choosing-approach"></a>
...
To choose from client implementations, see all the available Mainnet ready [execution clients](../#execution-clients), [consensus clients](../#consensus-clients) and learn about [client diversity](../client-diversity/).

Decide whether to run the software on your own [hardware or in the cloud](index.md#local-vs-cloud), considering clients' [requirements](index.md#requirements).

After preparing the environment, install the chosen clients either with [beginner-friendly interface](index.md#automatized-setup) or [manually](index.md#manual-setup) using a terminal with advanced options.

When the node is running and syncing, you are ready to [use it](index.md#using-the-node), but make sure to keep an eye on its [maintenance](index.md#operating-the-node).

![Client setup](../../../../public/content/developers/docs/nodes-and-clients/run-a-node/diagram.png)
...
Both options have different advantages summed up above. If you are looking for a cloud solution, in addition to many traditional cloud computing providers, there are also services focused on deploying nodes. Check out [nodes as a service](../nodes-as-a-service/) for more options on hosted nodes.
...
The size of the database and speed of the initial synchronization depends on the chosen client, its configuration and [sync strategy](../#sync-modes).
...
### Spinning up the node <a href="#spinning-up-node" id="spinning-up-node"></a>
...
#### Guided setup <a href="#automatized-setup" id="automatized-setup"></a>
...
#### Manual clients setup <a href="#manual-setup" id="manual-setup"></a>
...
First, you need to obtain your preferred [execution client](../#execution-clients) and [consensus client](../#consensus-clients) software.
...
It is also worth noting that client diversity is an [issue on the execution layer](../client-diversity/#execution-layer). It is recommended that readers consider running a minority execution client.
...
[Client diversity](../client-diversity/) is critical for consensus nodes running validators. If the majority of validators are running a single client implementation, network security is at risk. It is therefore recommended to consider choosing a minority client.

[See the latest network client usage](https://clientdiversity.org/) and learn more about [client diversity](../client-diversity/).
...
For testing purposes, you might prefer to run a client on one of the testnet networks. [See overview of supported networks](../#execution-clients).
...
* Specifies network to connect to, Mainnet in our examples
  * You can instead choose [one of testnets](../../networks/) for preliminary testing of your setup
...
#### Adding Validators <a href="#adding-validators" id="adding-validators"></a>
...
#### Using the node <a href="#using-the-node" id="using-the-node"></a>

Execution clients offer [RPC API endpoints](../../apis/json-rpc/) that you can use to submit transactions, interact with or deploy smart contracts on the Ethereum network in various ways:
...
Running your own node lets you use services that require direct access to Ethereum client RPC. These are services built on top of Ethereum like [layer 2 solutions](../../scaling/#layer-2-scaling), backend for wallets, block explorers, developer tools and other Ethereum infrastructure.

**Monitoring the node**

To properly monitor your node, consider collecting metrics. Clients provide metrics endpoints so you can get comprehensive data about your node. Use tools like [InfluxDB](https://www.influxdata.com/get-influxdb/) or [Prometheus](https://prometheus.io/) to create databases which you can turn into visualizations and charts in software like [Grafana](https://grafana.com/). There are many setups for using this software and different Grafana dashboards for you to visualise your node and the network as a whole. For example, check out [tutorial on monitoring Geth](../../../tutorials/monitoring-geth-with-influxdb-and-grafana/).
